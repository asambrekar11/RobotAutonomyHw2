    ...
    def Plan(self, start_config, goal_config, epsilon = .02):
        ...
        while connection_reached == False :
            ...
            if vf.all() == False:  
               ...
               is_path, q_newf = self.planning_env.Extend(q_nearf,q_rand,epsilon)
               if is_path != False:
               ...
               is_path, q_newf = self.planning_env.Extend(q_nearf,q_nearr,epsilon)
               if is_path != False:
                  sf = ftree.AddVertex(q_newf)
                  ftree.AddEdge(vidf,sf)               
                  greedf=True
            ...
            if vr.all() == False:  
               ...
               is_path, q_newr = self.planning_env.Extend(q_nearr,q_rand,epsilon)
               if is_path != False:
               ...
               is_path, q_newr = self.planning_env.Extend(q_nearr,q_nearf,epsilon)
               if is_path != False:
                  sr = rtree.AddVertex(q_newr)
                  rtree.AddEdge(vidr,sr)  
                  rcheck=q_newr             
